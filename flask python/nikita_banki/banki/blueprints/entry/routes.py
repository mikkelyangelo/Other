...
def choose():
    ...
    if request.method == 'GET':
        ...
        print(id)
        sql = provider.get('add_item.sql', id=DishID)
        print(sql)
...
def add_to_basket(bask, item):
    ...
    # return True
    bask[item['DishID']] = {'DishName': item['DishName'], 'DishCost': item['DishCost'],
                                      'amount': 1}
...
def save_order():
    ...
    if basket:
        sql = provider.get('insert_order.sql', user_date=datetime.now().strftime("%Y−%m−%d−%H:%M:"))
