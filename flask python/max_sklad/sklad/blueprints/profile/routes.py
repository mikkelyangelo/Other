...
def get_sql1():
    if request.method == 'GET':
        ...
        if year is not None:
            ...
            if not result:
                return render_template('not_found.html', str=result)
...
def get_sql2():
    if request.method == 'GET':
        ...
        month = request.form.get('month', None)
        if year is not None:
            sql = provider.get('task2.sql', year=year, month=month)
            result = work_with_db(db_config, sql)
            if not result:
                return render_template('not_found.html', str=result)
...
def get_sql3():
    if request.method == 'GET':
        ...
        if year is not None and month is not None:
            sql = provider.get('task3.sql', year=year, month=month)
            result = work_with_db(db_config, sql)
            if not result:
                return render_template('not_found.html', str=result)
...
def get_sql4():
    if request.method == 'GET':
        ...
        if prefix is not None:
            sql = provider.get('task4.sql', prefix=prefix)
            result = work_with_db(db_config, sql)
            if not result:
                return render_template('not_found.html', str=result)
