...
def login_page():
    if request.method == 'GET':
        ...
        if group_user:
            ...
            sql = provider.get('sql2.sql', login=login, password=password)
            id = work_with_db(db_config, sql)
            if id:
                user_info = ["empty", id[0]['id']]
                session['group_name'] = user_info[0]
                session['login'] = user_info[1]
                return render_template('confirm.html', str='Вы успешно авторизовались!', group_user=user_info[0])
            else:
                return render_template('login.html', str='Ошибка авторизации, попробуйте еще раз!')
