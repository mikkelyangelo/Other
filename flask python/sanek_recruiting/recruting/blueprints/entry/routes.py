...
def choose():
    ...
    if request.method == 'GET':
        ...
        print(job_id)
        sql = provider.get('add_item.sql', id=job_id)
        print(sql)
...
def add_to_basket(bask, item):
    ...
    # return True
    bask[item['job_id']] = {'job_name': item['job_name'], 'min_salary': item['min_salary'], 'max_salary': item['max_salary'],
                                      'amount': 1}
...
def save_order():
    ...
    for key in basket.keys():
        item = basket[key]
        sql = provider.get('insert_order_list.sql', order_id=order_id, job_id=key)
        insert_into_db(db_config, sql)
    basket = {}
