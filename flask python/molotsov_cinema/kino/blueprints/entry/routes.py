...
def edit_index():
    ...
    if request.method == 'GET':
        ...
        return render_template('edit.html', items=result, heads=["Дата  и время показа", "Режиссёр","Студия","Год","Страна"])
    else:
...
def choose():
    ...
    if request.method == 'GET':
        ...
        sql = provider.get('all_items.sql', id=session_id)
...
def add_to_basket(bask, item):
    ...
    # return True
    bask[item['t_id']] = { 't_row': item['t_row'],'t_place': item['t_place'], 't_price' : item['t_price'],
                                      'amount': 1}
