...
def create_1():
    if request.method == 'GET':
        ...
        find_check_count = provider.get(reports[0]['sql_check'], month=month, year=year, day=day)
        check_count = work_with_db(db_config, find_check_count)
        if check_count[0]['count'] == 0:
            sql = provider.get(reports[0]['sql_create'], month=month, year=year, day=day)
...
def view_1():
    if request.method == 'GET':
        ...
        if check_count[0]['count'] != 0:
            sql = provider.get(reports[0]['sql_view'], month=month, year=year, day=day)
            result = select_dict(db_config, sql)
            title = f"Отчёт за {month} месяц {year} года {day} дня"
